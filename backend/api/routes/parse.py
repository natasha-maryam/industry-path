...
@router.post("", response_model=ParseBatchResponse)
def parse_project(project_id: str, payload: ParseBatchRequest | None = None) -> ParseBatchResponse:
    file_ids = payload.file_ids if payload else []
    return ParseBatchResponse(**parse_service.parse_project(project_id, file_ids=file_ids))
