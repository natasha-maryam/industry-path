...
const toEngineeringRow = (row: UNSRow): EngineeringTableResponseRow => ({
  ...
  upstream_links: (row.upstream ?? []).map((tag) => ({ tag, provenance: "explicit", inferred: false })),
  downstream_links: (row.downstream ?? []).map((tag) => ({ tag, provenance: "explicit", inferred: false })),
